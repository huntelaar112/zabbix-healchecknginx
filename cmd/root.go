...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.healcheck_nginx.yaml)")
...
func initConfig() {
	...
	if !utils.PathIsExist(filepath.Join(home, ".healcheck_nginx.toml")) {
		...
		if nginx_ip == "" {
			...
			//fmt.Println(home)
			_, err = utils.FileCreateWithContent(filepath.Join(home, ".healcheck_nginx.toml"), []byte("NginxContainerIP="+"'http://"+nginx_ip+"/status/bestatus'"))
			if err != nil {
				Logger.Error(err)
			}
		}
